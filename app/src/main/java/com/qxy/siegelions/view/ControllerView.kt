...
import com.qxy.siegelions.util.AutoLinkHerfManager
import com.qxy.siegelions.util.NumUtils
import com.qxy.siegelions.util.OnVideoControllerListener
...
class ControllerView(context: Context?, attrs: AttributeSet?) : RelativeLayout(context, attrs), View.OnClickListener {
    ...
    private var videoData: Video? = null
    ...
    fun setVideoData(videoData: Video) {
        this.videoData = videoData
        ivHead!!.setImageResource(videoData.user!!.head)
        tvNickname!!.text = "@" + videoData.user!!.nickName
        AutoLinkHerfManager.setContent(videoData.content, autoLinkTextView)
        ivHeadAnim!!.setImageResource(videoData.user!!.head)
